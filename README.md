...
└── README.md
```
# Week 4 — Fine-Tuning TrOCR on Urdu Dataset

**Code Saviours SI-26 | ML/AI Internship | Muhammad Ahmad**

## Overview

This week's goal was to fine-tune Microsoft's pretrained `trocr-base-printed` model on a custom Urdu dataset, evaluate its accuracy, and save the result. The notebook (`SI26_Week4_Muhammad_Ahmad.ipynb`) is fully self-contained — it uploads the dataset, builds the training pipeline, trains, evaluates, and exports everything needed for GitHub, in one file.

## Dataset

| Category | Approx. images | Notes |
|---|---|---|
| synthetic (original, Week 1/3) | 114 | Rendered text, fully labeled |
| synthetic_gen (new, Week 4) | 600 | Generated from a bank of 282 unique sentences using the Noto Nastaliq Urdu font, fully labeled automatically |
| signboards / newspaper / other | 111 | Real-world images, labeling in progress |

The notebook includes a synthetic data generator (Section 4.5) that renders additional labeled Urdu training images instantly, with varied fonts, sizes, colors, and slight rotation/blur — since the text is generated by the notebook itself, every label is automatically correct.

## Pipeline Summary

1. Upload and extract the dataset zip
2. Filter to rows with real text labels (blank rows are excluded from training)
3. Load `microsoft/trocr-base-printed`
4. **Extend the tokenizer/model vocabulary** to support Urdu characters (see below)
5. Fine-tune for 30 epochs with gradient clipping and a linear warmup/decay learning rate schedule
6. Track validation loss every epoch and keep the best-performing checkpoint
7. Evaluate on a held-out test set using both exact-match accuracy and Character Error Rate (CER)
8. Save the fine-tuned model to Google Drive
9. Export the updated dataset (original + generated images + labels) for GitHub

## Key Technical Issues Found and Fixed

This week involved substantial debugging beyond the base handout instructions. Documenting these here because they were the most instructive part of the assignment:

**1. Tokenizer instantiation crash**
`TrOCRProcessor.from_pretrained(...)` failed with a backend tokenizer error on newer `transformers` versions. Fixed by pinning `transformers==4.46.3` and loading the tokenizer with `use_fast=False`.

**2. English-only vocabulary**
`trocr-base-printed` was pretrained entirely on English text. Its tokenizer represents unfamiliar scripts through raw byte-level fallback, meaning each Urdu character was being split into multiple disconnected byte-fragments — extremely hard for the model to learn to reproduce correctly. Fixed by scanning the dataset for every unique Urdu character, adding them as native tokens via `tokenizer.add_tokens()`, and resizing the model's embedding layers to match.

**3. Padding not excluded from the loss (the most significant bug)**
Labels were padded to a fixed length of 128 tokens, but real text is typically 15–45 characters — meaning roughly 80–90% of every training label was padding. Without masking, the loss function graded the model equally on "predict padding correctly" and "predict the real character correctly," letting the model achieve deceptively low loss by learning to mostly output padding instead of actually reading the image. Fixed by replacing padding token ids with `-100` in the labels so the loss function ignores them, forcing the model to focus entirely on real characters.

**4. Evaluating on the wrong checkpoint**
Validation loss did not always improve monotonically — occasionally the final epoch was worse than an earlier one. Since evaluation ran on whatever weights existed at the end of training, this meant occasionally grading the model on a worse checkpoint than it had actually reached. Fixed by tracking validation loss every epoch and restoring the best-performing weights before final evaluation.

**5. Overly aggressive generation settings**
An attempt to fix repeated-character output during generation (using `repetition_penalty` and `no_repeat_ngram_size`) was tested and found to *worsen* results rather than help. Reverted to plain beam search (`num_beams=4`) without repetition penalties.

## Metrics Reported

- **Exact-match accuracy**: percentage of predictions matching the true text exactly, character for character (strict).
- **Character Error Rate (CER)**: edit-distance-based measure of how many character insertions/deletions/substitutions would be needed to correct a prediction. More informative for OCR since it credits close-but-imperfect predictions rather than scoring them as a total failure.

## Results

- **Training**: 30 epochs, with automatic best-checkpoint restoration by validation loss (see `training_loss.png` for the full curve)
- **Exact-match accuracy**: 4.2% (6/143 test images predicted perfectly, character for character)
- **Character Error Rate (CER)**: 22.9%
- **Character-level accuracy**: 77.1%

This is a substantial improvement over earlier attempts during development (which ranged from 0% up to ~32% character-level accuracy before the dataset was expanded to 282 unique sentences and training was extended). Many incorrect predictions are now visibly close to the correct answer - missing or substituting only a handful of characters in an otherwise correct sentence - rather than the unrelated/garbled output seen in earlier debugging stages.

## Honest Notes for Week 5

- Most training data is still synthetic; the 111 real-world images (signboards, newspaper clippings) are labeled only partially. Testing this model on real-world photos (rather than clean synthetic renders) is the natural next step, and finishing that labeling is the highest-value way to see how well this generalizes beyond synthetic text.
- Two changes had the biggest measurable impact on accuracy: (1) fixing the padding-mask bug in the loss function, and (2) expanding the sentence bank from 66 to 282 unique sentences combined with more training epochs. Both were identified by reading the actual failure patterns in the model's output rather than guessing.
- Exact-match accuracy (4.2%) is still much lower than character-level accuracy (77.1%) - the model is getting most characters right per sentence but rarely a full sentence perfectly. This gap is expected for a five-week internship project and is a reasonable discussion point for Week 5: what would it take to close that gap (more data, more real-world variety, longer training, or a different accuracy metric altogether)?

## How to Run

1. Open `SI26_Week4_Muhammad_Ahmad.ipynb` in Google Colab
...
- [Hugging Face Transformers](https://huggingface.co/docs/transformers) (`transformers==4.46.3`)
