...
| synthetic_gen (new, Week 4) | 300+ | Generated on demand using the Noto Nastaliq Urdu font, fully labeled automatically |
...
5. Fine-tune for 20 epochs with gradient clipping and a linear warmup/decay learning rate schedule
...
- **Training loss**: went from approximately X (epoch 1) to X (final epoch) — see `training_loss.png`
- **Validation loss**: best value X at epoch X
- **Exact-match accuracy**: X%
- **Character-level accuracy**: X%

*(Fill in with your actual run's numbers before submitting.)*

## Honest Notes for Week 5

- Most training data is still synthetic; the 111 real-world images (signboards, newspaper clippings) are labeled only partially. Finishing that labeling is the single highest-value next step for improving accuracy.
- Given the dataset size and Colab's free-tier training budget, results at this stage should be read as "a working, debugged pipeline producing a real (if modest) result" rather than a polished production model — which is a reasonable and honest place to be at Week 4 of 5.
